...
| <a name="input_blob_storage_backup"></a> [blob\_storage\_backup](#input\_blob\_storage\_backup) | blob storage backup policy settings for the storage account. Defaults to null which does not set any blob storage backup policy | <pre>object({<br>    name             = string<br>    backup_vault_id  = string<br>    backup_policy_id = string<br>  })</pre> | `null` | no |
